...
class solver_s336733_s343674(AbstractSolver):
                    ...
                    for constructor in (self._pack_items_floor, self._pack_items_fast_stack):
                        ...
                        if valid:
                            ...
                            if cost + EPS < best_cost:
                                best_cost = cost
                                best_bins = bins
                    fixed_vehicles = self._economical_large_vehicles()
                    if len(self.items) > 1200:
                        fixed_vehicles = fixed_vehicles[:2]
                    for vehicle in fixed_vehicles:
                        bins = self._pack_items_fixed_vehicle_fast_stack(ordered_items, vehicle)
                        if bins is None:
                            continue
                        valid, _ = self.validate_solution(bins)
                        if valid:
    ...
    def _economical_large_vehicles(self) -> List[VehicleData]:
        candidates = []
        ranked_groups = (
            sorted(self.vehicles, key=lambda v: (v.cost / max(v.max_weight, EPS), v.cost)),
            sorted(self.vehicles, key=lambda v: (v.cost / max(v.volume, EPS), v.cost)),
            sorted(self.vehicles, key=lambda v: (v.cost, -v.max_weight)),
        )
        seen = set()
        for group in ranked_groups:
            for vehicle in group[:4]:
                if vehicle.type not in seen:
                    seen.add(vehicle.type)
                    candidates.append(vehicle)
        return candidates

    def _pack_items_fixed_vehicle_fast_stack(self, ordered_items: Sequence[ItemData], vehicle: VehicleData) -> Optional[List[BinState]]:
        bins: List[BinState] = []
        for item in ordered_items:
            best = None
            candidate_bins = sorted(
                bins,
                key=lambda b: (-(b.used_volume / max(b.vehicle.volume, EPS)), b.used_weight, len(b.boxes))
            )
            if len(candidate_bins) > 40:
                candidate_bins = candidate_bins[:40]
            ...
            if best is not None:
                self._add_box(best[1], best[2])
                continue
            if item.weight > vehicle.max_weight + EPS or item.value > vehicle.max_value + EPS:
                return None
            new_bin = BinState(idx=len(bins), vehicle=vehicle)
            candidates = list(self._candidate_boxes_fast_stack(item, new_bin))
            if not candidates:
                return None
            box, _score = min(candidates, key=lambda pair: pair[1])
            self._add_box(new_bin, box)
            bins.append(new_bin)
        return bins
    ...
    def _fast_stack_points(self, bin_state: BinState) -> List[Tuple[float, float, float]]:
        ...
        return sorted(set(filtered), key=lambda p: (p[2], p[1], p[0]))[:100]
